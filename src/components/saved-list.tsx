...
export function SavedList() {
  ...
  return <>{result.events.length < ids.length ? <p className="inline-message">현재 제공하지 않는 행사 일부는 표시되지 않을 수 있어요.</p> : null}<div className="event-grid saved-grid">{result.events.map((event, index) => <EventCard key={event.id} event={event} filters={{ date: nextSaturday(), region: "전체", category: "전체", sort: "recommended" }} index={index} />)}</div></>;
}
