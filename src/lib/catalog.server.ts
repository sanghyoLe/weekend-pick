...
import { districtFromAddress, distanceKm, recommend, type EventItem, type Filters, type Region } from "./domain";
...
export async function getDistricts(region: Region): Promise<string[]> {
  if (region === "전체") return [];
  if (isDemo()) return [...new Set(demoEvents().filter(event => event.region === region).map(withDistrict).map(event => event.district))].sort();
