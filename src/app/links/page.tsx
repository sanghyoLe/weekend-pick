...
import { addDays, nextSaturday, todayKST } from "@/lib/domain";
...
const events = [
  {
    title: "서울거리예술축제",
    meta: (saturday: string) => `${shortDate(saturday)}–${shortDate(addDays(saturday, 1))} · 서울 광진·성동`,
    note: "뚝섬한강공원·서울숲 / 무료",
    href: "https://www.sfac.or.kr/site/SFAC_KOR/12/11204040000002025040909.jsp",
  },
  {
    title: "수원재즈페스티벌",
    meta: (saturday: string) => `${shortDate(addDays(saturday, -1))}–${shortDate(saturday)} · 경기 수원`,
    note: "광교호수공원 재미난밭 / 무료",
    href: "https://www.suwon.go.kr/sw-www/deptHome/dep_tour/culture05/culture05_03/culture05_03_03.jsp",
  },
  {
    title: "시흥갯골축제",
    meta: (saturday: string) => `${shortDate(addDays(saturday, -1))}–${shortDate(addDays(saturday, 1))} · 경기 시흥`,
    note: "시흥갯골생태공원 / 입장 무료",
    href: "https://shggfestival.com/",
  },
  {
    title: "부천국제만화축제",
    meta: (saturday: string) => `${shortDate(addDays(saturday, -1))}–${shortDate(addDays(saturday, 1))} · 경기 부천`,
    note: "한국만화박물관 일원 / 일반 5,000원",
    href: "https://bicof.com/",
  },
  {
    title: "광주시 남한산성문화제",
    meta: (saturday: string) => `${shortDate(addDays(saturday, -1))}–${shortDate(addDays(saturday, 1))} · 경기 광주`,
    note: "남한산성도립공원 일원 / 무료",
...
function periodLabel(saturday: string): string {
  const friday = addDays(saturday, -1);
  const sunday = addDays(saturday, 1);
  const start = new Date(`${friday}T00:00:00Z`);
  const end = new Date(`${sunday}T00:00:00Z`);
  return `${start.getUTCMonth() + 1}월 ${start.getUTCDate()}일(${weekdays[start.getUTCDay()]}) — ${end.getUTCDate()}일(${weekdays[end.getUTCDay()]})`;
}

export default function LinksPage() {
  const saturday = nextSaturday(todayKST());
  return (
    <main id="main" className="page-width links-page">
      <header className="links-hero">
        ...
        <div className="links-period"><CalendarDays size={15} aria-hidden />{periodLabel(saturday)}</div>
      </header>
